...
public class SkeletonBoss1 : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        {
            // Beräkna önskad hastighet
            Vector2 targetVelocity = new Vector2(walkSpeed * 10, rb.velocity.y);

            // Begränsa hastigheten till önskad hastighet
            rb.velocity = Vector2.ClampMagnitude(targetVelocity, walkSpeed * 10);
        }
        else if (targetRange.isChasing && canWalk && !isDead)
        {
            ...
            // Beräkna önskad hastighet
            Vector2 targetVelocity = new Vector2(direction.x * chaseSpeed * 10, rb.velocity.y);

            // Begränsa hastigheten till önskad hastighet
            rb.velocity = Vector2.ClampMagnitude(targetVelocity, chaseSpeed * 10);
        }
